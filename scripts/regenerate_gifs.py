"""Regenerate PDF evolution GIFs with higher resolution and a hold at the final frame."""
import subprocess, os
from PIL import Image

def regenerate_gif(mp4_path, gif_path, fps=6, hold_sec=8.0, scale=1500):
    ...
    subprocess.run([
        ...
        "-filter_complex",
        f"[0:v]fps={fps},scale={scale}:-1:flags=lanczos[x];"
...
if __name__ == "__main__":
    ...
    for name in ["pdf_evolution", "pdf_evolution_barrier"]:
        ...
        apng = os.path.join(base, f"{name}.apng")
        if os.path.exists(mp4):
            # Remove old APNG if present
