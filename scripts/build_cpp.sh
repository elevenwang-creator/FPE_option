...
echo "=== [2/5] Installing C/C++ header ==="
...
mojo build -I src --emit shared-lib -o "_fpe_native${PY_EXT_SUFFIX}" src/bindings/_fpe_native.mojo

echo "=== [4/5] Installing Python package into site-packages ==="
mkdir -p "${SITE_PACKAGES}/fpe_engine"
cp "_fpe_native${PY_EXT_SUFFIX}" "${SITE_PACKAGES}/fpe_engine/"
