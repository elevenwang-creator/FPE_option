...
struct TensorProductBasis[degree_s: Int, degree_v: Int](Copyable, Movable):
    ...
    def eval_tensor(self, s_points: List[Float64], v_points: List[Float64]) -> CSRMatrix[DType.float64]:
        var Bs = self.basis_s.eval_all(s_points)
        var Bv = self.basis_v.eval_all(v_points)
        return kron(Bs, Bv)

    def partial_s(self, s_points: List[Float64], v_points: List[Float64]) -> CSRMatrix[DType.float64]:
        var dBs = self.basis_s.first_derivative_all(s_points)
        var Bv = self.basis_v.eval_all(v_points)
        return kron(dBs, Bv)

    def partial_v(self, s_points: List[Float64], v_points: List[Float64]) -> CSRMatrix[DType.float64]:
        var Bs = self.basis_s.eval_all(s_points)
