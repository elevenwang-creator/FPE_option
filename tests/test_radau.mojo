...
def main() raises:
    ...
    if not sol1.success:
        ...
        for i in range(n):
            ...
            if err > max_err:
                max_err = err
            print("  y[" + String(i) + "] = " + String(y_final[i]) +
                  "  exact = " + String(exact[i]) +
                  "  err = " + String(err))
    ...
    if not sol2.success:
        ...
        for i in range(n):
            ...
            if err > max_err2:
                max_err2 = err
            print("  y[" + String(i) + "] = " + String(y_final2[i]) +
                  "  exact = " + String(exact2[i]) +
                  "  err = " + String(err))
    ...
    print("  sum(y(t)) = exp(-t) * sum(y0) => should decrease but stay positive")
    print()
    ...
    if not sol3.success:
        ...
        print("  relative error = " + String(abs_f64(y_sum - exact_sum) / exact_sum))
    ...
    print("  E1_h = U1*M + h*K, size=" + String(E1_h.nrows) + "x" + String(E1_h.ncols) +
          ", nnz=" + String(E1_h.nnz()))
    ...
    if not sol5.success:
        ...
        var exact5: List[Float64] = [exp(-0.1 * 5.0), exp(-0.5 * 5.0), exp(-2.0 * 5.0)]
        var max_rel_err5 = 0.0
        for i in range(n):
            var rel_err = abs_f64(y_final5[i] - exact5[i]) / max_f64(1e-10, abs_f64(exact5[i]))
            if rel_err > max_rel_err5:
                max_rel_err5 = rel_err
            print("  y[" + String(i) + "] = " + String(y_final5[i]) +
                  "  exact = " + String(exact5[i]) +
    ...
    var solver6 = RadauSparseLinearSolver[SimpleLinearSystem](
        rtol=1e-3, atol=1e-6, first_step=0.01,
    )
