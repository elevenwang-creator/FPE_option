"""Mojo benchmark with large matrix (n=100).

Uses a tridiagonal K matrix (n=100) for realistic performance testing.
"""
...
def main() raises:
    print("=" * 70)
    print("  Mojo Large Matrix Benchmark (n=100)")
    ...
    def bench_large_mojo() raises capturing:
        var sol = solver.solve(sys, (0.0, 0.5), y0)
        _ = sol

    print("[1] Large system (n=100, t=0→0.5):")
    var report = bench_run[bench_large_mojo](
        num_warmup_iters=3,
        max_iters=10,
    ...
    print("=" * 70)
    print("  Large matrix benchmark complete")
    print("=" * 70)
