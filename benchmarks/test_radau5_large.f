C Fortran 77 large matrix benchmark (n=100)
C Uses a tridiagonal K matrix for realistic performance testing
C Compiles with macOS Accelerate framework

      PROGRAM RADAU5_LARGE_MATRIX
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      INTEGER I, J, K_STEP, NEWT, ITERS
      PARAMETER (N=100, NS=3, MAX_STEPS=100)
...
C RADAU5 coefficients from Hairer's code
     ...
     &           DD3=-1.0D0/3.0D0)
      ...
      PRINT *, '='//REPEAT('=',68)
      PRINT *, '  Fortran 77 Large Matrix Benchmark (n=100)'
...
C Benchmark
     ...
     &         IPIV, INFO, DGETRF, DGETRS, U1, DD1, DD2, DD3)
      ...
      PRINT *, '[1] Large system (n=100, t=0→0.5):'
      ...
      PRINT *, '='//REPEAT('=',68)
      PRINT *, '  Fortran Large Matrix Benchmark complete'
      PRINT *, '='//REPEAT('=',68)
...
C Main integration loop (50 steps from 0→0.5, h=0.01)
      DO K_STEP = 1, 50
