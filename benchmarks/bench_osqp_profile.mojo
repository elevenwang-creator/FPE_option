"""Profile OSQP.solve_nnls_sparse internals to find sub-bottleneck."""

from engines.fpe.domain import FPEDomain, FPECachedBasis
from engines.fpe.heston_params import HestonParams
from engines.fpe.galerkin import mass_from_cached
from engines.fpe.initial_cond import _diag_inv_sqrt_vec, _elem_mul
...
def main() raises:
    ...
    # Replicate OSQP.solve_nnls_sparse internals
    ...
    var t1 = now()
    print(" 1. M.transpose():  ", Float64(t1 - t0) / 1e9, "s nnz:", Mt.nnz())

    # 2. Mt @ M (spgemm)
    t0 = now()
    var MtM = Mt @ galerkin_scaled
    t1 = now()
    print(" 2. Mt @ M:         ", Float64(t1 - t0) / 1e9, "s nnz:", MtM.nnz())

    # 3. In-place diagonal shift
    t0 = now()
    var alpha = lambda_reg + sigma + rho
    ...
    t1 = now()
    print(" 4. to_csc:         ", Float64(t1 - t0) / 1e9, "s nnz:", KKT_csc.nnz())
    ...
    kron_T_spmv_dual(
        ...
        weights_spmv(cached.s_weights, cached.v_weights, ones),
    ...
    # 7. ADMM iterations (measure total)
    ...
    var eps_abs = 1e-8
    var eps_rel = 1e-8
    from std.math import sqrt, abs, max as fmax
    ...
    for iter in range(max_iter):
        ...
        while i + width <= n:
            ...
            var sr = sq + rho * (sz - su) + sigma * sx
            (rhs.ptr() + i).store[width=width](sr)
            i += width
        while i < n:
            rhs[i] = q[i] + rho * (z[i] - u[i]) + sigma * x[i]
            i += 1
        ...
        var eps_dual = eps_abs + eps_rel * rho * sqrt(u_norm_sq)
        iters_used = iter + 1
    ...
    t1 = now()
    print(" 7. ADMM loop:     ", Float64(t1 - t0) / 1e9, "s iters:", iters_used)
